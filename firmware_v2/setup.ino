...
// --- Reset cause -------------------------------------------------------------
// The RA4M1 latches why it last reset in RSTSR0/1/2 (hardware registers, so they
// survive the reset). We snapshot them at the start of setup(), show the decoded
// cause, then clear the flags so the next boot describes only the next reset.
// A plain power-on (PORF) is the boring case; anything else (watchdog, brownout/
// LVD, software) gets an OLED splash.
//
...
void setup() {  
 // global init
  ...
  // timeout, so it's safe to run before the watchdog is armed. The firmware
  // update check is NOT done at boot — it's triggered at runtime with main + up
  // (see handle_combo_main_up), so the boot path can never hang on a flaky
  // network or a wedged modem.
  setup_net();
...
void setup_buttons() {
  ...
  button.setDoubleClickHandler(handle_main_double);
  button.setTripleClickHandler(handle_main_triple);
